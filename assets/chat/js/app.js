...
/* [FR] Ajoute une variante sans accent pour chaque mot-cle enregistre. */
const addAccentlessAliases = () => {
    Object.keys(keywordMap).forEach(key => {
        const plain = deaccent(key);
        if (plain !== key && !keywordMap[plain]) keywordMap[plain] = keywordMap[key];
    });
};
...
const typeWriterEffect = (element, text) => {
    return new Promise(resolve => {
        ...
        const typewriter = new Typewriter(element, {
            ...
            stringSplitter: (str) => {
                scrollToBottom(); 
                /* [CORRECTIF] Le texte situe a l'interieur d'une balise
                   (<strong>, <li>...) est ré-échappé une seconde fois par la
                   librairie : "&" finissait par s'afficher en "&amp;".
                   On le decode ici, juste avant la frappe. */
                const decoded = str
                    .replace(/&amp;/g, '&')
                    .replace(/&nbsp;/g, '\u00a0')
                    .replace(/&quot;/g, '"')
...
const scrollToBottom = () => {
    /* [CORRECTIF] Le chat est ici dans un panneau (#ai-chat-overlay) :
       c'est LUI qui defile, pas la fenetre. Sinon les nouvelles reponses
       n'etaient pas suivies automatiquement. */
    const panel = document.getElementById('ai-chat-overlay');
